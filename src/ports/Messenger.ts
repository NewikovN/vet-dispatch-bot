...
export interface Messenger {
  ...
  /**
   * Простое текстовое уведомление в беседу (не карточка — без клавиатуры, ничего не
   * редактирует и не привязывается к message_id заявки/вакцинации в БД). Нужно для событий,
   * которые легко потерять в потоке других сообщений, если просто перерисовать карточку
   * (та же причина, что у markGroupCardMoved) — например, закрытие заявки/вакцинации.
   */
